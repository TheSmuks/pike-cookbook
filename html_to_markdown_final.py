...
def html_to_markdown(html_file_path, md_file_path):
    ...
    # Find start from "9. Directories" (in H1)
    start_pattern = r'<H1 CLASS="SECT1"><A NAME="DIRECTORIES">9\. Directories</A></H1>'
    start_match = re.search(start_pattern, content)
    if not start_match:
        print("Could not find Directories section start")
        return
    ...
    # Find next H2 (major section break)
    next_h2 = re.search(r'<H2 CLASS="SECT2">', content[start_pos:])
    if next_h2:
        end_positions.append(start_pos + next_h2.start())
